...
$submit = "<input class='sub' type='submit'>";
...
$obj = new test();
$usertype = $obj->get_user_type($USER->id,$course->id);

if ($usertype == "Manager") {

	if (isset($_POST['Accept'])) {
		$arr = array();
		$groups = $DB->get_records('groups',array());
		 foreach ($groups as $key => $value) {
            ...
            foreach ($arr as $group) {
                if($_POST[$group])
                {
                	$obj->manager_response($group,$USER->id,'Acceptance','I Accept your Invitation to join to this group');
                	
                	$obj->add_user_to_group($group,$USER->username);
                	$obj->delete_message($group,$USER->id);
                	break;
		...
		$groups = $DB->get_records('groups',array());
		 foreach ($groups as $key => $value) {
            ...
            foreach ($arr as $group) {
                if($_POST[$group])
                {
                	$obj->manager_response($group,$USER->id,'Reject',"Sorry I can't Join to your group");
                	$obj->delete_message($group,$USER->id);
                	break;
	...
	$groups = $obj->select_invitation($USER->id);
	if (!empty($groups)) {
		...
		foreach ($groups as $group) { 
			...
			<textarea rows="1" cols="25" name="<?php echo $group; ?>" readonly> Group Name: <?php echo $group; ?></textarea>
...
if($usertype == "Student")
{
    ...
    if(isset($_POST['Invite']))
    {
    	...
    	$user = $obj->get_user($_POST['manager']);
        $obj->send_message($USER,$user,$_SESSION["POST_VARS"]['gname'],'Invitation','I invite You to Accept my Invitation to our group',$_SESSION["POST_VARS"]['gname']);

    }
    
  
    
    $memberin = $obj->get_student_groups($USER->id);
    //echo(($memberin[1])->id);
    $arr = array();

    foreach ($memberin as $key => $value) {
        array_push($arr,($value->name));
    }
    if(empty($arr))
    {
     ...
     if(isset($_POST['Select']))
        {
            ...
            $i=1;
             $managers = $obj->select_managers($_POST['gname'],$course->id);
             if(empty($managers))
             {
             	$i=0;
           ...
           <select name="manager">
            ...
            <?php  
                ...
                foreach ($managers as $manager) { ?>
                        ...
                        <option name="<?php $manager; ?>"><?php echo $manager ?></option>
               <?php } ?>
...
<?php
   ...
   ?>
        ...
        <form  action="#" method="POST">
           <select name="gname">
           	<option name="Select Group">Select Group</option>

            <?php  
            
                foreach ($arr as $key) { ?>

                        <option name="<?php $key; ?>"><?php echo $key ?></option>
               <?php } ?>
...

{
     $k = 0;
         ...
         if(isset($_POST['submit']))
                    {
                    	if($_POST['groupname'] !== "Select Group")
                    	{
              ...
              $_SESSION["POST_VARS"]= $_POST;
                ...
                $get = new test();
                         $k=1;
              $notmembers = $get->getstudent($_POST['groupname'],$course->id);
              ...
              {
              	$k=0;
              	echo "There is no Student to Add to this Group";
              }
              else
              {
             ...
             <select name="usernames">
                <?php 
                    foreach ($notmembers as $student) { 
                       
                                ?>

                     <option name="<?php $student; ?>"><?php echo $student ; ?> </option>
                        

               <?php } ?><input type="submit" value="add" name="add">              
        </select>
...
if($k==0)
{
    ...
    <form  action="#" method="POST">
        <select name="groupname">
            ...
            <?php  
            $get= new test();
           $groups = $get->getgroups($course->shortname);?>
                    <option name="Select Group">Select Group</option>
                    <?php
            foreach ($groups as $key) { ?>
                        
                        <option name="<?php $key->name; ?>"><?php echo $key->name ?></option>
               <?php } ?>
  ...
  if(isset($_POST['add']))
{
    ...
    $add = new test();
    if (!empty($_SESSION["POST_VARS"]["groupname"]))
    
     {
        if(!empty($_POST['usernames']))
         {
         	$add->add_user_to_group($_SESSION["POST_VARS"]["groupname"],$_POST['usernames']);
         	echo "Student ".$_POST['usernames']." Added to ".$_SESSION["POST_VARS"]["groupname"]." Suceesfully";
 		}
